...
class ImageMembershipRepo(object):

    def __init__(self, context, db_api, image=None, tenant=None):
        self.context = context
        self.db_api = db_api
        self.image_id = image.image_id if image else None
        self.member_id = tenant.tenant_id if tenant else None
    ...
    def _format_image_member_to_db(self, image_member):
        image_member = {'image_id': image_member.image_id,
                        'member': image_member.member_id,
                        'created_at': image_member.created_at}
        return image_member

    def list_members(self, image_id=None, member_id=None):
        image_id = self.image_id or image_id
        member_id = self.member_id or member_id
    ...
    def list_images(self, image_repo, image_id=None, member_id=None):
        image_id = self.image_id or image_id
        member_id = self.member_id or member_id
        db_members = self.db_api.image_member_find(self.context,
                                                   image_id=image_id,
        ...
        for db_member in db_members:
            try:
                images.append(image_repo.get(db_member['image_id']))
