...
class TestImagesController(test_utils.BaseTestCase):
    ...
    def test_index_member_status_accepted_owner(self):
        self.config(limit_param_default=5, api_limit_max=5)
        request = unit_test_utils.get_fake_request(tenant=TENANT3)
        output = self.controller.index(request, owned_by=TENANT4)
        self.assertEqual(4, len(output['images']))
        actual = set([image.image_id for image in output['images']])
        expected = set([UUID1, UUID2, UUID3, UUID4])
        self.assertEqual(actual, expected)

    def test_index_member_status_rejected_owner(self):
        self.config(limit_param_default=5, api_limit_max=5)
        request = unit_test_utils.get_fake_request(tenant=TENANT3)
        output = self.controller.index(request, owned_by=TENANT4,
...
class TestImagesDeserializer(test_utils.BaseTestCase):
    ...
    def test_index_owned_by(self):
        request = unit_test_utils.get_fake_request('/images?owned_by=me')
    ...
    def test_index_member_status_owned_by(self):
        path = '/images?member_status=pending&owned_by=me'
        request = unit_test_utils.get_fake_request(path)
        expected = {'sort_key': 'created_at',
                    'member_status': 'pending',
                    'owned_by': 'me',
