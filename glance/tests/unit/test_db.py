...
class TestImageRepo(test_utils.BaseTestCase):
    ...
    def test_list_status_pending(self):
        self.context = glance.context.RequestContext(
                        user=USER1, tenant=TENANT3)
        self.image_repo = glance.db.ImageRepo(self.context, self.db)
        images = self.image_repo.list(member_status='pending')
        self.assertEqual(2, len(images))

    def test_list_status_pending_owner(self):
        self.context = glance.context.RequestContext(
                        user=USER1, tenant=TENANT3)
        self.image_repo = glance.db.ImageRepo(self.context, self.db)
        images = self.image_repo.list(member_status='pending',
    ...
    def test_list_status_pending_owner_none(self):
        self.context = glance.context.RequestContext(
                        user=USER1, tenant=TENANT3)
        self.image_repo = glance.db.ImageRepo(self.context, self.db)
        images = self.image_repo.list(member_status='pending',
    ...
    def test_list_status_rejected(self):
        self.context = glance.context.RequestContext(
                        user=USER1, tenant=TENANT3)
        self.image_repo = glance.db.ImageRepo(self.context, self.db)
        images = self.image_repo.list(member_status='rejected')
        self.assertEqual(2, len(images))
    ...
    def test_list_status_all(self):
        self.context = glance.context.RequestContext(
                        user=USER1, tenant=TENANT3)
        self.image_repo = glance.db.ImageRepo(self.context, self.db)
        images = self.image_repo.list(member_status='all')
        self.assertEqual(3, len(images))
