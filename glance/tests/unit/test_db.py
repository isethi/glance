...
class TestImageRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image_repo_member = glance.db.ImageRepo(self.context, self.db,
                                                     TENANT2)
    ...
    def test_list_images_for_member(self):
        images = self.image_repo_member.list_images_for_member()
        actual = set([image.image_id for image in images])
        expected = set([UUID1])
